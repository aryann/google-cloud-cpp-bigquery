...
class AdminClient {
 public:
  explicit AdminClient(std::shared_ptr<AdminConnection> conn)
      : conn_(std::move(conn)) {}
  ...
  /// Creates a new Dataset with the name \p dataset_id in the project
  /// referenced by \p project_id.
...
std::shared_ptr<AdminConnection> MakeAdminConnection(
